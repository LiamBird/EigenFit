...
def mpt_to_df(filename, eis=False):
    """
    Converts .mpt files from Biologic (BT-lab or EC-lab) electrochemical cycler to pandas dataframe. 
    Inputs:
    ----------
    filename (str):
        The filename including path to convert
        
    eis (bool) optional: default = False
        Separates EIS measurements where repeated EIS spectra are present in a .mpt file
    """
