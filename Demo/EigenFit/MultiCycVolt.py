...
#         for scan_rate in self.data.keys():
#             for cycle in range(self.data[scan_rate].n_cycles):
#                 peak_currents[scan_rate].update([(cycle, [])])
#                 peak_voltages[scan_rate].update([(cycle, [])])
#                 try:
#                     I = vars(self.data[scan_rate])[cycle_type].current[cycle]/1000
#                     V = vars(self.data[scan_rate])[cycle_type].voltage[cycle]
#                     dIdV = (I[step:]-I[:-step])/(V[step:]-V[:-step])
#                     d2IdV2 = (dIdV[step:]-dIdV[:-step])/(V[2*step:]-V[:-2*step])

#                     for ni, i in enumerate(dIdV[step:-step]):
#                         if cycle_type=="discharge" and np.sign(I[step+ni]) == -1 and np.sign(dIdV[ni-1])==1 and np.sign(dIdV[ni])==-1 and np.sign(d2IdV2[step+ni])==1:
#                             peak_currents[scan_rate][cycle].append(I[step+ni])
#                             peak_voltages[scan_rate][cycle].append(V[step+ni])
#                         if cycle_type=="charge" and np.sign(I[step+ni])==1 and np.sign(dIdV[ni-1])==+1 and np.sign(dIdV[ni])==-1 and np.sign(d2IdV2[step+ni])==-1:
#                             peak_currents[scan_rate][cycle].append(I[step+ni])
#                             peak_voltages[scan_rate][cycle].append(V[step+ni])
#                 except:
#                     pass
...
#                     selected_voltages[scan_rate].append(np.nan)
#                     selected_currents[scan_rate].append(np.nan)
#         combined_xs = [item for sublist in [[np.sqrt(keys)]*len(values) for keys, values in selected_currents.items()] for item in sublist]
#         combined_ys = [item for sublist in [*selected_currents.values()] for item in sublist]            
