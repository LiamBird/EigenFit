...
class SpectrumMap(object):
    ...
    def __init__(self, data_file, reverse_shift=True):
        ...
        self.x_coords = np.array(np.unique(raw_data[:, 0]), dtype=float) ## changed 01/05/2026
        self.y_coords = np.array(np.unique(raw_data[:, 1]), dtype=float) ## changed 01/05/2026
    ...
    def slide_viewer(self, show="raw"):
        """
        Returns an ipywidgets controlled interactive graph display, with widgets to control x and y position of spectrum to display
            ...
            Parameters:
                show (str): one of "raw", "clipped", or "ccd"
        ...
        shift = vars(self)[show].shift
        map_data = vars(self)[show].map
        
        spectrum, = ax.plot(shift, map_data[0, 0, :])
        
        x_slider = widgets.IntSlider(min=0, max=self.x_extent-1, value=0)
        y_slider = widgets.IntSlider(min=0, max=self.y_extent-1, value=0)
        ...
        def update(change=None):
            ...
            spectrum.set_data(shift, ydata)
            ax.set_ylim(ydata.min()*0.9, ydata.max()*1.1)
    ...
    def _slide_viewer_legacy(self, display="raw"):
        """
        Returns an ipywidgets controlled interactive graph display, with widgets to control x and y position of spectrum to display
